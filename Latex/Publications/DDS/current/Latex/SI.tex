...
\begin{document}
  ...
  % \begin{equation}
  %   \sumch \ \intlm v \left(\dcDt - D \ddcDDx \right) \Am \ \dx = 0.
  % \end{equation}
  ...
  % \begin{equation}
  %   \sumch \ \intlm \left( v \dcDt + \dvDx D \dcDx \right) \Am \ \dx=  -\sumch \left[ v \Am j \right]_{x=0}^{\lm}.
  % \end{equation}
  ...
  % \begin{equation}
  %   \sumch \ \intlm \left( v \dcDt + \dvDx D \dcDx \right) \Am \ \dx=  \vr\JR - \vc\JC.
  % \end{equation}
  % The particle current at the reservoir boundary $\JR$ is obtained by the rate of change of particles in the reservoir:
  % \begin{equation}
  %   \JR = -\frac{\partial}{\partial t} (\cR\VR), 
  % \end{equation}
  % where $\cR$ and $\VR$ are the concentration and volume of the reservoir, respectively. For the cavity an additional particle current $\Jex$ removing particles from the cavity due to liquid exchange with the surrounding body is considered. Thus one has:
  % \begin{equation}
  %   \JC = \frac{\partial}{\partial t} (\cC\VC) + \Jex \qquad \text{with} \qquad \Jex = \cC Q,
  % \end{equation} 
  % where $\cC$ and $\VC$ are the concentration and volume of the cavity, respectively and $Q = \partial\Vex/\partial t$ is the volume current of exchanged liquid. Note that a homogeneous concentration both in the reservoir and the cavity and a constant volume current is assumed. With that one obtains:

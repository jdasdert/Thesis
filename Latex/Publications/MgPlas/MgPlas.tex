...
\subsubsection{Residuals and algorithmic tangent}
  ...
  \begin{equation}
    \gammareli = 
    \begin{cases}
      \displaystyle \frac{\sumbI \summ \cb \abs{\D\gammaab}}{\sumbI \cb \D\gammaaccb + \sumbI \D\gammab} \qquad \text{(slip modes)}\\[15pt]
      \displaystyle \frac{\sum_{\beta \in I^i} \D\gammab}{\sumbI \cb \D\gammaaccb + \sumbI \D\gammab} \qquad \text{(twinning modes)}
    \end{cases}
  \end{equation}
  ...
  The partial derivatives of $\EEe$ with respect to the unknowns $\SSe$ and $\D\gammautilde$ needed for the linearization of the residual are given by:
  ...
  \begin{align}
    ...
    \dEEeDDeltagammau &= \sumbO \sumaI \dEEeDDeltagammaab \dDeltagammaabDtauacb \dtauacbDDeltagammau + \sumbI \dEEeDDeltagammab \left(\dDeltagammabDxibc \dxibcDDeltagammau + \derivative{\D\gammab}{c_0}\derivative{c_0}{\D\gammau} \right)
  \end{align}
  ...
  and
  \begin{equation}
      \dhhp =  \sumbO \cb \sumaI \MMab \d \D \gammaab + \sumbI \left(\MMb + \frac{1}{\gammatwb}\sumaI\left(\MMab\D\gammaab-\MMaO\D\gammaaO\right)\right)  \d \D \gammab
      \label{eq:dhhp}
  \end{equation}
  and 
  \begin{equation}
  ...
  one obtains the partial derivatives of $\EEe$ with respect to the plastic shear and twinning increments $\D\gammaab$ and $\D\gammab$ and the elastic trial Green strain tensor $\CCetr$
  \begin{align}
    \dEEeDDeltagammaab &= - \cb \sym{\CCe \MMabtilde} \label{eq:dEEeDDeltagammaab}\\
    \dEEeDDeltagammab &= - \sym{\CCe \left(\MMbtilde+\frac{1}{\gammatwb}\sumaI \left(\MMabtilde\Delta\gammaab-\MMaOtilde\D\gammaab\right)\right)} \label{eq:dEEeDDeltagammab}\\
    \derivative{\EEe}{\CCetr} &= \half \ffpinvT \sBox \ffpinv
  \end{align}
  
  The partial derivatives of $\D\gammaab$ and $\D\gammab$ are given by 
  \begin{align}
    \derivative{\D\gammaab}{\tauab} &= \frac{\Dt \nu_0 \psl}{\tauacb} \left(\frac{\abs{\tauab}}{\tauacb} \right)^{\psl-1} \\
    \derivative{\D\gammab}{\xib} &= c_0 \gammatwb \frac{\Dt \nu_0 \ptw}{\xibc} \left(\frac{\xib}{\xibc} \right)^{\ptw-1} \\
    \derivative{\D\gammab}{c_0} &= \Dt \nu_0 \left(\frac{\xib}{\xibc} \right)^{\ptw}
  \end{align}
  and
  \begin{align}
    \derivative{\D\gammaab}{\tauacb} &= - \frac{\tauab}{\tauacb} \derivative{\D\gammaab}{\tauab} \\
    \derivative{\D\gammab}{\xibc} &= -\frac{\xib}{\xibc} \derivative{\D\gammab}{\xib}
  \end{align}
  
  The linearization for $\tauCslslb$, $\tauCttsl$ and $\tauCctsl$ reads: 
  \begin{align}
          \d\tauCslslb &= 
          \begin{cases}  \displaystyle h_0 \ \d\D\gammaaccb & \qquad \qquad \text{(Basal)} 
            \\ \displaystyle h_0 \ \sech^2\left(\frac{h_0 \gammaaccb}{\taus - \tauO}\right) \d\D\gammaaccb & \qquad \qquad \text{(Non basal)}
          \end{cases}\\
  ...
  \begin{align}
    \derivative{\tauacb}{\D\gammaaccb} & = \Ab \derivative{\tauCslslb}{\D\gammaaccb} \\[10pt]
    \derivative{\tauacb}{\D\gammab} & = \derivative{\Ab}{\D\gammab} (\tauCslslb + \tauCttslb + \tauCctsl)
  \end{align}
  with
  \begin{equation}
    \derivative{\Ab}{\D\gammab} = 
    \begin{cases}
  ...
  \begin{equation}
    \derivative{\xibc}{\D\gammab} = 
    \begin{cases}
      \displaystyle A_0 \gammatt \hO \ \sech^2\left(\frac{\hO \gammaacctt}{\taus-\tauO}\right) + \derivative{A_0}{\D\gammab}  \gammatt (\taus-\tauO) \tanh{\frac{\hO \gammaacctt}{\taus-\tauO}} & \qquad \text{(TT)} \\[10pt]
      \displaystyle \gammact \hO (\cb)^{b} + \derivative{A_0}{\D\gammab} \gammact \frac{\hO (\gammaaccct)^{b+1}}{(b+1){(\gammact})^b} & \qquad \text{(CT)} 
    \end{cases}
  \end{equation}
  ...
  The partial derivatives of $\gammau$ with respect to the unknowns $\SSe$ and $\D\gammau$ needed for the linearization of the residual are given by:
  ...
  \begin{align}
    ...
    \dDeltagammauDDeltagammau &= \sumbO \sumaI \dDeltagammauDDeltagammaab \dDeltagammaabDtauacb \dtauacbDDeltagammau + \sumbI \dDeltagammauDDeltagammab \left( \dDeltagammabDxibc \dxibcDDeltagammau + \derivative{\D\gammab}{c_0}\derivative{c_0}{\D\gammau} \right)
  \end{align}
  

  The partial derivatives are then given by:
  \begin{align}
    \frac{\partial \D\gammau_i}{\partial \D\gammaab} &= \delta_{i\beta} \ \sign{\tauab} \\
    \frac{\partial \D\gammau_i}{\partial \D\gammab} &= \delta_{ij} \quad \text{with} \quad j=\Ntw+1+\beta
  \end{align}
  ...
  \begin{align}
      \derivative{\SSebar}{\SSe} &= \sumbO \sumaI \derivative{\SSebar}{\D \gammaab} \derivative{\D \gammaab}{\tauab} \derivative{\tauab}{\SSe}
      + \sumbI \derivative{\SSebar}{\D \gammab}\derivative{\D \gammab}{\xib}\derivative{\xib}{\SSe} + \ffpinv \sBox \ffpinvT;\\
      \derivative{\SSebar}{\gammau} &= \sumbO \sumaI \derivative{\SSebar}{\D \gammaab} \derivative{\D \gammaab}{\tauacb} \derivative{\tauacb}{\gammau}
      + \sumbI \derivative{\SSebar}{\D \gammab} \left(\dDeltagammabDxibc \dxibcDDeltagammau + \derivative{\D\gammab}{c_0}\derivative{c_0}{\D\gammau} \right).
  \end{align}
  
  It is calculated from the linearization of $\SSebar$ as follows:
  \begin{align}
      \d \SSebar = \d \left( \ffpinv \SSe \ffpinvT \right) = \ffpinv \sBox \ffpinvT : \left( \d\SSe + 2 \sym{\SSe\trans{(\ffp\d\ffpinv)}} \right).
  \end{align}
  ...
  \begin{align}
    ...
    \derivative{\SSebar}{\gammab} &= - \ffpinv \sBox \ffpinvT : 2 \sym{ \SSe \left(\trans{\MMbtilde}+\frac{1}{\gammatwb}\sumaI\left(\trans{\MMabtilde} \D\gammaab-\trans{\MMaOtilde}\D\gammaaO\right)\right)}.
  \end{align}

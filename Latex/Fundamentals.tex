...
\begin{figure}[h!]
  ...
  \caption[]{Balance of an arbitrary quantity $Z$ can be given for a finite Volume $V$ or locally for an infinitesimal volume $\d V$.}
  \label{fig:Balance}
\end{figure}
...
\begin{align}
    \nu = \nu_0 \, \sign{\tau} \left( \frac{\abs{\tau}}{\tau^\c} \right)^{p}
    \label{eq:powerlaw}
\end{align}
...
\end{align}
where $\epsilon$ is the material dependent permittivity and $\varphi$ is the electric potential. For electric currents due to electrons $\jj^\e$ Ohm's law is assumed:
\begin{align}
  \jj^\e = -\sigma^\e \, \grad \varphi
\end{align}
where $\sigma^\e$ is the material specific conductivity.
